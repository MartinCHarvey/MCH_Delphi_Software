...
uses
  SharedSymmetries, SysUtils, SyncObjs, CellCountShared, Generics.Collections,
...
{$ENDIF}
  ...
  TPackedRepSearchVal = class //Not used in this version.
  end;

  TCanonicalDict = TDictionary<TSymBoardPackedState, TCellCountMTBoard>;
  //TODO - Specify comparer for dict?
  TCanonicalComparer = class (TEqualityComparer<TSymBoardPackedState>)
    //TODO - Override functions here to check for equality and hash?
  end;
...
{$ENDIF}
  public
    FCanonicalHashDict: TCanonicalDict;
    FMinLexList: TObjectList;
  private
    FLock: TMultiReadExclusiveWriteSynchronizer;
    FTraverseIndex: integer;
...
function SortMinlex(A, B: Pointer): integer;
var
  OwnBoard, OtherBoard: TCellCountMTBoard;
begin
  OwnBoard := TCellCountMTBoard(B);
  OtherBoard := TCellCountMTBoard(A);
...
var
  Enum: TCanonicalDict.TValueEnumerator;
  Val: TCellCountMTBoard;
begin
  if FTraverseState = ttsAdding then
  begin
    Assert(FMinlexList.Count = 0);
    //Take all items in the hash dict, add to list,
    //sort it, clear the hash dict.
    Enum := FCanonicalHashDict.Values.GetEnumerator;
    //Enumerator not initially focussed?
    try
      while Enum.MoveNext do
      begin
        Val := Enum.Current;
        Assert(Assigned(Val));
        Assert(Val is TCellCountMTBoard);
        FMinlexlist.Add(Val);
      end;
    finally
      Enum.Free;
    end;
    FCanonicalHashDict.Clear;
    FMinLexList.Sort(SortMinLex);
  end;
...
begin
  inherited;
  //TODO - Default hashing ok?
  //FCanonicalHashDict := TCanonicalDict.Create(TCanonicalComparer.Create);
  FCanonicalHashDict := TCanonicalDict.Create(0);
  FMinLexList := TObjectList.Create;
  FMinLexList.OwnsObjects := true;
...
begin
  FCanonicalHashDict.Free;
  FMinlexList.Free; //Should free objects.
  FLock.Free;
  inherited;
end;
...
begin
  Existing := nil;
  if FCanonicalHashDict.TryGetValue(Board.CanonicalRep, Existing) then
  begin
  ...
  begin
    ...
    begin
      FCanonicalHashDict.Remove(Board.CanonicalRep);
      Existing.Free;
    end;
    FCanonicalHashDict.Add(Board.CanonicalRep, Board);
    result := abrAdded;
  end;
end;
...
begin
  ...
  try
    if FTraverseState = ttsAdding then
      result := FCanonicalHashDict.Count
    else
      result := FMinLexList.Count;
  finally
    FLock.EndRead;
  end;
end;
...
begin
  ...
  try
    ...
    case FTraverseState of
      ...
      ttsTraversing:
      begin
        if FTraverseIndex < FMinLexList.Count then
        begin
          result := TCellCountMTBoard(FMinLexList.Items[FTraverseIndex]);
          Inc(FTraverseIndex);
        end
        else
        begin
